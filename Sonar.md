...
| Полоса | B | ≤ B_tr |
| Длительность | Tp | Ограничено физически: Tp ≤ 0.8 × TOF(D_min) |
...
Система поддерживает две стратегии оптимизации параметров:

#### 7.1.1 Стратегия 1: Максимальный Tp + Минимальный VGA Gain (Max Tp + Min VGA)

**Принцип работы:**
1. Рассчитывает максимально допустимую длительность импульса (Tp) как 80% времени туда-обратно для целевой дальности D_target.
2. Если текущий Tp меньше максимального, рекомендует увеличить Tp до максимального значения.
3. Если текущий Tp уже на максимуме или выше, использует текущий Tp.
4. Для выбранного Tp рассчитывает минимальный VGA Gain, необходимый для достижения целевого SNR:
   - Оценивает SNR при оптимальном Tp (SNR масштабируется как 10*log10(Tp)).
   - Рассчитывает требуемое изменение VGA Gain для достижения целевого SNR.
   - Если SNR при оптимальном Tp уже выше целевого, может рекомендовать уменьшить VGA Gain.

**Преимущества:**
- Максимизирует энергию сигнала за счет использования максимальной длительности импульса.
- Минимизирует усиление VGA, что может снизить шум и энергопотребление.
- Подходит для случаев, когда важна максимальная точность измерения.

**Когда использовать:**
- Когда требуется максимальная точность измерения дальности.
- Когда доступна достаточная полоса пропускания для длинных импульсов.
- Когда важно минимизировать шум приемного тракта.

#### 7.1.2 Стратегия 2: Минимальный Tp для целевого SNR (Min Tp for SNR)

**Принцип работы:**
1. Рассчитывает минимальную длительность импульса (Tp), необходимую для достижения целевого SNR на целевой дальности D_target.
2. Использует уравнение активного сонара для точного расчета минимального Tp:
   - SNR = SL + TS + DI - NL - 2TL + 10*log10(BT)
   - Где BT = bandwidth * Tp (произведение полосы пропускания на длительность)
   - Из уравнения находится минимальный Tp для заданного SNR
3. Применяет физические ограничения:
   - Tp не может превышать 80% времени туда-обратно для D_target
   - Если D_target > D_min, Tp может быть больше, чем 80% TOF для D_min
   - Если D_target ≤ D_min, Tp ограничен 80% TOF для D_min

**Преимущества:**
- Минимизирует длительность импульса, что улучшает разрешение по дальности.
- Позволяет быстрее выполнять измерения (меньше время ожидания между импульсами).
- Эффективно использует доступную полосу пропускания.
- Использует точное уравнение сонара для расчета оптимального Tp.

**Когда использовать:**
- Когда требуется высокое разрешение по дальности.
- Когда важна скорость измерений.
- Когда доступна ограниченная полоса пропускания.

**Выбор стратегии:**

Выбор стратегии оптимизации доступен в правой панели GUI под виджетом Signal Path. Стратегия применяется автоматически при запуске оптимизации параметров.

### 7.2 Процесс оптимизации

1. CORE выполняет симуляцию.
2. Если условия нарушены, CORE возвращает failure flag + рекомендации.
3. GUI / внешний optimizer корректирует DTO согласно выбранной стратегии:
...
### 7.3 Time Gain Control (TGC)

**Назначение:**

TGC (Time Gain Control) - это техника автоматической регулировки усиления VGA в зависимости от расстояния до цели. График TGC показывает, как должен изменяться VGA gain с расстоянием для поддержания оптимальных условий приёма сигнала.

**Принцип работы:**

1. **Компенсация затухания сигнала:**
   - Сигнал затухает с расстоянием из-за spreading loss и absorption loss
   - Round-trip transmission loss: TL_total = 2 × TL (туда и обратно)
   - TGC компенсирует это затухание: G_VGA ≈ 2 × TL

2. **Обеспечение целевого SNR:**
   - Для каждой дистанции рассчитывается SNR на выходе LNA (без VGA)
   - Определяется требуемый VGA gain для достижения целевого SNR
   - Если SNR уже достаточен, VGA gain может быть уменьшен

3. **Избежание клиппинга ADC:**
   - Рассчитывается максимальный VGA gain, при котором сигнал не превышает полную шкалу ADC
   - VGA gain ограничивается для предотвращения перегрузки ADC

4. **Учёт ограничений VGA:**
   - VGA gain ограничен диапазоном [G_min, G_max] выбранной модели VGA
   - График показывает эти ограничения в виде горизонтальных линий

**Формула расчёта:**

Для каждой дистанции D:

1. Transmission Loss (one-way): TL = Spreading Loss + Absorption Loss
2. SNR на выходе LNA: SNR_LNA = SL + TS + DI - NL - 2×TL + 10×log₁₀(BT)
3. Требуемый VGA gain для SNR: G_VGA_SNR = max(0, target_SNR - SNR_LNA)
4. TGC компенсация: G_VGA_TGC = 2 × TL
5. Максимальный VGA gain (без клиппинга): G_VGA_max = 20×log₁₀(V_ADC_max / V_LNA)
6. Оптимальный VGA gain: G_VGA = min(max(G_VGA_SNR, G_VGA_TGC), G_VGA_max, G_VGA_max_range)

**График TGC:**

График отображается в отдельном TAB "TGC" в GUI и показывает:
- Зависимость VGA gain от дистанции (синяя линия)
- Горизонтальные линии ограничений VGA (G_min - зелёная, G_max - красная)
- Автоматическое обновление после каждой симуляции

**Применение:**

TGC кривая используется для:
- Настройки системы автоматической регулировки усиления
- Планирования параметров приёмного тракта
- Оптимизации динамического диапазона системы
- Обеспечения постоянного уровня сигнала на входе ADC независимо от расстояния
...
6. Учитываем эффект корреляции (12.9.6).
...
- ✅ График "Depth vs Tp": зависимость максимальной достижимой глубины от длительности импульса для заданного SNR
- ✅ График TGC (Time Gain Control): зависимость оптимального VGA gain от дистанции для поддержания целевого SNR и избежания клиппинга
